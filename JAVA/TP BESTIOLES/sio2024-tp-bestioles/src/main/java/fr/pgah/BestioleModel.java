...
public class BestioleModel {
  // Cette constante indique à quelle fréquence INFECTER devrait réussir
  // pour les bestioles qui n'ont pas sauté au mouvement précédent.
  // 0.0 : infecte à tous les coups
  // 1.0 : n'infecte jamais
  public static final double RESISTANCE = 0.2; // 20 % de résistance
  ...
  public void ajouterBestioles(int nb, Class<? extends Bestiole> bestiole) {
    ...
    String name = bestiole.getSimpleName();
  ...
  private void InitialiserBestiole(Class<? extends Bestiole> bestiole,
      Bestiole.Direction[] directions) {
    ...
    try {
      ...
      System.out
          .println("ERREUR : " + bestiole + " a lancé une exception depuis son constructeur.");
      System.exit(1);
    }
  ...
  private Bestiole instancierBestiole(Class<? extends Bestiole> bestiole) throws Exception {
    Constructor c = bestiole.getConstructors()[0];
    // String debug = bestiole.toString();
    if (bestiole.toString().equals("class fr.pgah.Ours")) {
  ...
  private Info recupererInfo(PrivateData data, Class<? extends Bestiole> original) {
    ...
    return new Info(coord, neighbors, data.direction, neighborThreats);
  }

  private Bestiole.Voisin getVoisin(Point p, Class<? extends Bestiole> original) {
    if (!inBounds(p))
  ...
  public void update() {
    ...
    for (int i = 0; i < bestioles.length; i++) {
      ...
      if (move == Bestiole.Action.GAUCHE)
        ...
        if (inBounds(p2) && grille[p2.x][p2.y] != null
            && grille[p2.x][p2.y].getClass() != next.getClass()
            && !locked.contains(grille[p2.x][p2.y]) && (hadHopped || Math.random() >= RESISTANCE)) {
          Bestiole other = grille[p2.x][p2.y];
          PrivateData oldData = infosBestioles.get(other);
          String c1 = other.getClass().getSimpleName();
          comptageBestioles.put(c1, comptageBestioles.get(c1) - 1);
          String c2 = next.getClass().getSimpleName();
  ...
  private static class Info implements BestioleInfo {
    ...
    public Info(Point coord, Bestiole.Voisin[] neighbors, Bestiole.Direction dir,
